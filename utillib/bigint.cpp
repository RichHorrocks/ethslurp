...
//------------------------------------------------------------
bigint_t::bigint_t(void)
: sign(0), mag()
{
}

//------------------------------------------------------------
bigint_t::bigint_t(const bigint_t &x)
...
//------------------------------------------------------------
bigint_t::bigint_t(const biguint_t &x)
: mag(x)
{
	sign = (mag.len==0) ? 0 : 1;
}
...
//------------------------------------------------------------
bigint_t::bigint_t(const unsigned long *b, unsigned int len)
: mag(b, len)
{
	sign = (mag.len==0) ? 0 : 1;
}
...
//------------------------------------------------------------
bigint_t::bigint_t(const unsigned long *b, unsigned int blen, int sgn)
: mag(b, blen)
{
	switch (sgn)
	{
	case 0:
		if (mag.len!=0)
			throw "bigint_t::bigint_t(const unsigned long *, unsigned int, int): Cannot use a sign of zero with a nonzero magnitude";
		sign = 0;
		break;
...
//------------------------------------------------------------
bigint_t::bigint_t(const biguint_t &x, int sgn)
: mag(x)
{
	switch (sgn)
	{
	case 0:
		if (mag.len!=0)
			throw "bigint_t::bigint_t(const biguint_t &, int): Cannot use a sign of zero with a nonzero magnitude";
		sign = 0;
		break;
...
//------------------------------------------------------------
bigint_t::bigint_t(unsigned long  x)
: mag(x)
{
	sign = ((mag.len==0) ? 0 : 1);
}
...
//------------------------------------------------------------
bigint_t::bigint_t(unsigned int   x)
: mag(x)
{
	sign = ((mag.len==0) ? 0 : 1);
}
...
//------------------------------------------------------------
bigint_t::bigint_t(unsigned short x)
: mag(x)
{
	sign = ((mag.len==0) ? 0 : 1);
}
...
namespace
{
	...
	unsigned long magOf(X x)
	{
		/* UX(...) cast needed to stop short(-2^15), which negates to
		 * itself, from sign-extending in the conversion to unsigned long. */
		return ((unsigned long)(x < 0 ? UX(-x) : x));
	}
	template <class X>
	int signOf(X x) {
		return (x == 0) ? 0
...
//------------------------------------------------------------
bigint_t::bigint_t(long  x) : sign(signOf(x)), mag(magOf<long , unsigned long >(x)) {}
//------------------------------------------------------------
bigint_t::bigint_t(int   x) : sign(signOf(x)), mag(magOf<int  , unsigned int  >(x)) {}
//------------------------------------------------------------
bigint_t::bigint_t(short x) : sign(signOf(x)), mag(magOf<short, unsigned short>(x)) {}
...
X bigint_t::convertToSignedPrimitive() const
{
	...
	} else if (mag.len == 1)
	{
		...
		unsigned long b = mag.getBlock(0);
		if (sign == 1)
		{
			X x = X(b);
			if (x >= 0 && ((unsigned long)(x)) == b)
				return x;
		} else
		{
			X x = -X(b);
...
//------------------------------------------------------------
long bigint_t::to_long(void) const
{
	return convertToSignedPrimitive  <long , unsigned long> ();
}

//------------------------------------------------------------
int            bigint_t::to_int    ( void ) const { return convertToSignedPrimitive  <int  , unsigned int>  (); }
//------------------------------------------------------------
short          bigint_t::to_short  ( void ) const { return convertToSignedPrimitive  <short, unsigned short>(); }
//------------------------------------------------------------
unsigned long  bigint_t::to_ulong  ( void ) const { return convertToUnsignedPrimitive<unsigned long >       (); }
//------------------------------------------------------------
unsigned int   bigint_t::to_uint   ( void ) const { return convertToUnsignedPrimitive<unsigned int  >       (); }
//------------------------------------------------------------
unsigned short bigint_t::to_ushort ( void ) const { return convertToUnsignedPrimitive<unsigned short>       (); }
...
//------------------------------------------------------------
unsigned long bigint_t::getBlock(unsigned int i) const
{
