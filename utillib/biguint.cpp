...
//----------------------------------------------------------------------
biguint_t::biguint_t(void)
: BigNumStore<unsigned long>()
{
}

//----------------------------------------------------------------------
biguint_t::biguint_t(const biguint_t &x)
: BigNumStore<unsigned long>(x)
{
}
...
{
	BigNumStore<unsigned long>::operator=(x);
	return *this;
}

//----------------------------------------------------------------------
biguint_t::biguint_t(int unused, unsigned int c)
: BigNumStore<unsigned long>(0, c)
{
}

//----------------------------------------------------------------------
biguint_t::biguint_t(const unsigned long *b, unsigned int blen)
: BigNumStore<unsigned long>(b, blen)
{
	trimLeft();
}
...
biguint_t::biguint_t ( unsigned long  x ) {                          if (x!=0) {capacity=0;allocate(1);if(blk){len=1;blk[0]=(unsigned long)(x);}}}
biguint_t::biguint_t ( unsigned int   x ) {                          if (x!=0) {capacity=0;allocate(1);if(blk){len=1;blk[0]=(unsigned long)(x);}}}
biguint_t::biguint_t ( unsigned short x ) {                          if (x!=0) {capacity=0;allocate(1);if(blk){len=1;blk[0]=(unsigned long)(x);}}}
biguint_t::biguint_t (          long  x ) { if (x<0) throw msg; else if (x!=0) {capacity=0;allocate(1);if(blk){len=1;blk[0]=(unsigned long)(x);}}}
biguint_t::biguint_t (          int   x ) { if (x<0) throw msg; else if (x!=0) {capacity=0;allocate(1);if(blk){len=1;blk[0]=(unsigned long)(x);}}}
biguint_t::biguint_t (          short x ) { if (x<0) throw msg; else if (x!=0) {capacity=0;allocate(1);if(blk){len=1;blk[0]=(unsigned long)(x);}}}
...
//----------------------------------------------------------------------
unsigned long  biguint_t::to_ulong  ( void ) const       { return convertToPrimitive       <unsigned long > (); }
unsigned int   biguint_t::to_uint   ( void ) const       { return convertToPrimitive       <unsigned int  > (); }
unsigned short biguint_t::to_ushort ( void ) const       { return convertToPrimitive       <unsigned short> (); }
long           biguint_t::to_long   ( void ) const       { return convertToSignedPrimitive <         long > (); }
int            biguint_t::to_int    ( void ) const       { return convertToSignedPrimitive <         int  > (); }
short          biguint_t::to_short  ( void ) const       { return convertToSignedPrimitive <         short> (); }

//----------------------------------------------------------------------
void biguint_t::trimLeft(void)
{
...
//----------------------------------------------------------------------
void biguint_t::setBlock(unsigned int i, unsigned long newBlock)
{
	if (newBlock==0)
	{
		if (i<len)
...
unsigned int biguint_t::bitLength() const
{
	...
	{
		unsigned long leftmostBlock = getBlock(len - 1);
...
void biguint_t::setBit(unsigned int bi, bool newBit)
{
	...
	unsigned long block = getBlock(blockI), mask = ((unsigned long)(1)) << (bi % N);
 ...
 */
//----------------------------------------------------------------------
unsigned long getShiftedBlock(const biguint_t &num, unsigned int x, unsigned int y)
{
	unsigned long part1 = (x == 0 || y == 0) ? 0 : (num.blk[x - 1] >> (biguint_t::N - y));
	unsigned long part2 = (x == num.len) ? 0 : (num.blk[x] << y);
	return part1 | part2;
}
...
void biguint_t::multiply(const biguint_t &a, const biguint_t &b)
{
	...
	for (i = 0; i < a.len; i++)
	{
		...
		for (i2 = 0; i2 < N; i2++)
		{
			if ((a.blk[i] & (((unsigned long)(1)) << i2)) == 0)
				continue;
			/*
...
void biguint_t::divide(const biguint_t &b, biguint_t &q) {
	...
	unsigned long *subtractBuf = new unsigned long[len];
...
void biguint_t::shiftRight(const biguint_t &a, int b)
{
	...
	// Now (N * rightShiftBlocks - leftShiftBits) == b and 0 <= leftShiftBits < N.
	...
	// Now we're allocating a positive amount. + 1: room for high bits nudged left into another block
